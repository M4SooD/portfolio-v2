'use client';

import { Button } from '@/components/ui/Button';
import { FiDownload } from 'react-icons/fi';

import Socials from './Socials';
import Photo from './Photo';

const Hero = () => {
  return (
    <section className="h-full">
      <div className="container mx-auto h-full">
        <div className="flex flex-col xl:flex-row items-center justify-between xl:pt-8 xl:pb-24">
          <div className="text-center xl:text-left order-2 xl:order-0">
            <span className="text-xl">Creative Front-End Developer</span>
            <h1 className="h1 mb-6">
              Hello I&apos;m <br />
              <span className="text-accent">Masood Moosavi</span>
            </h1>
            <p className="max-w-125 mb-9 text-white/80">
              Building digital products, brands, and experiences with a focus on
              motion, interaction, and clean code
            </p>

            <div className="flex flex-col xl:flex-row items-center gap-8">
              <a
                href="/assets/Masood Moosavi - Front End Dev.pdf"
                download="Masood_Moosavi_CV.pdf"
              >
                <Button
                  variant="outline"
                  size="lg"
                  className="uppercase flex items-center gap-2 border-accent text-accent hover:bg-accent hover:text-[#1c1c22] transition-all duration-500 cursor-pointer"
                >
                  <span>Download CV</span>
                  <FiDownload className="text-xl animate-bounce" />
                </Button>
              </a>
              <div className="mb-8 xl:mb-0">
                <Socials
                  containerStyles="flex gap-6"
                  iconStyles="w-9 h-9 border border-accent rounded-full flex justify-center items-center text-accent text-base hover:bg-accent hover:text-[#1c1c22] hover:transition-all duration-500"
                />
              </div>
            </div>
          </div>

          <div className="order-1 xl:order-0 mb-8 xl:mb-0">
            <Photo />
          </div>
        </div>
      </div>
    </section>
  );
};

export default Hero;
